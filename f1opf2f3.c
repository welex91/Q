...
// STOP FUNC DECL
{
  ...
  switch ( f1_rec.fldtype ) { 
    case I8  : op_type = I4; break;
    case I4  : op_type = I2; break;
    default : go_BYE(-1); break;
  }

  status = get_fld_sz(op_type, &fldsz); cBYE(status);
  filesz = fldsz * nR;
  ...
  if ( strcmp(op, "unconcat") == 0 ) { 
    switch ( f1_rec.fldtype ) { 
      case I8 : 
        {
          ...
          for ( long long i = 0; i < nR; i++ ) { 
            unsigned long long valf1UI8 = f1UI8[i];
      ...
      case I4 : 
        {
          unsigned int *f1UI4 = (unsigned int *)f1_X;
          unsigned short *f2UI2 = (unsigned short *)f2_X;
          unsigned short *f3UI2 = (unsigned short *)f3_X;
          for ( long long i = 0; i < nR; i++ ) { 
            unsigned int valf1UI4 = f1UI4[i];
            f2UI2[i] = (unsigned short)(valf1UI4 >> 16);
            f3UI2[i] = (unsigned short)(valf1UI4 & 0x0000FFFF);
          }
        }
        break;
      default : 
    ...
    zero_fld_rec(&f2_rec); f2_rec.fldtype = op_type; 
    status = add_fld(tbl_id, f2, f2_ddir_id, f2_fileno, &f2_id, &f2_rec); cBYE(status);
    zero_fld_rec(&f3_rec); f3_rec.fldtype = op_type; 
