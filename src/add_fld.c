...
{
  int status = 0;
  unsigned int hashval, startidx;

  if ( ( fld == NULL ) || ( *fld == '\0' ) )  { go_BYE(-1); }
  // Pick a spot to start searching for an empty spot
  status = hash_string_UI4(fld, &hashval);
  startidx = mix_I8(((unsigned long long)tbl_id << 32) | hashval);
...
// STOP FUNC DECL
{
  ...
  *ptr_fld_id = -1;
  ...
  status = get_empty_fld(tbl_id, fld, g_flds, g_n_fld, &fld_id); cBYE(status);
