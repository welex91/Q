...
#include "s_to_f_random_I4.h"
#include "s_to_f_random_I8.h"
...
#define MAX_LEN 32
  ...
  char  distrib[MAX_LEN];
  bool is_null = true;
  int nT; long long block_size;
  long long randlbI8, randubI8, seed;
  ...
  //----------------------------------------
  if ( strcmp(op, "random") == 0 ) {
    status = extract_S(str_scalar, "distrib=[", "]", distrib,MAX_LEN,&is_null); 
    cBYE(status); if ( is_null ) { go_BYE(-1); }
    if ( strcmp(distrib, "uniform") != 0 ) { go_BYE(-1); }

    status = extract_I8(str_scalar, "lb=[", "]", &randlbI8, &is_null); 
    cBYE(status);
    if ( is_null ) { go_BYE(status); }

    status = extract_I8(str_scalar, "ub=[", "]", &randubI8, &is_null); 
    cBYE(status);
    if ( is_null ) { go_BYE(status); }
    if ( randlbI8 >= randubI8 ) { go_BYE(-1); }

    status = extract_I8(str_scalar, "seed=[", "]", &seed, &is_null); 
    cBYE(status);
    if ( is_null ) { go_BYE(status); }
  ...
  for ( int tid = 0; tid < nT; tid++ ) { // POTENTIAL CILK LOOP
    ...
    if ( strcmp(op, "random") == 0 ) {
      if ( strcmp(fldtype, "I4") == 0 ) {
        if ( ( randlbI8 < INT_MIN ) || ( randlbI8 > INT_MAX ) ) { 
         status = -1; continue; 
        }
        if ( ( randubI8 < INT_MIN ) || ( randubI8 > INT_MAX ) ) { 
         status = -1; continue; 
        }
	XI4 = (int *)X; XI4 += lb;
	status = s_to_f_random_I4(XI4, nX, (int)randlbI8, (int)randubI8, 0);
      }
      else if ( strcmp(fldtype, "I8") == 0 ) {
	XI8 = (long long *)X; XI8 += lb;
	status = s_to_f_random_I8(XI8, nX, (int)randlbI8, (int)randubI8, 0);
      }
