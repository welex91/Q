...
// STOP FUNC DECL
{
  ...
  /*----------------------------------------------------*/
  status = process_aux_info(in_aux_info, &ignore_hdr, &fld_sep,
  &fld_open, &fld_close, &rec_delim);
  cBYE(status);

  status = read_csv(data_dir, data_file, fld_sep, fld_open, fld_close,
    rec_delim, ignore_hdr, flds, 
  ...
  for ( int i = 0; i < n_flds; i++ ) { 
    ...
    if ( fld_props[i].is_lkp == true ) {
      if ( fld_props[i].is_dict_old ) { 
        int dict_fld_id = -1, nn_dict_fld_id, tmp_tbl_id = -1;
        FLD_REC_TYPE dict_fld_rec, nn_dict_fld_rec;
	status = is_fld(NULL, dict_tbl_id[i], "txt", &dict_fld_id, 
            &dict_fld_rec, &nn_dict_fld_id, &nn_dict_fld_rec); cBYE(status);
        chk_range(dict_fld_id, 0, g_n_fld);
        tmp_tbl_id = dict_fld_rec.tbl_id;
        chk_range(tmp_tbl_id, 0, g_n_tbl);
	sprintf(strbuf, "%d:%d", tmp_tbl_id, dict_fld_id);
	status = int_set_meta(tbl_id, fld_id, "dict_fld", strbuf, true);
	cBYE(status);
