...
// STOP FUNC DECL
{
  ...
  else if ( strcmp(argv[1], "prime_geq") == 0 ) {
    if ( argc != 3 ) { go_BYE(-1); }
    int itemp;
    status = stoI4(argv[2], &itemp); cBYE(status);
    if ( itemp <= 1 ) { go_BYE(-1); }
    itemp = prime_geq(itemp);
    fprintf(stdout, "%d", itemp);
  }
  else if ( strcmp(argv[1], "qhead") == 0 ) {
    if ( argc != 5 ) { go_BYE(-1); }
    status = qhead(argv[2], argv[3], argv[4]); cBYE(status);
  }
  else if ( strcmp(argv[1], "behead") == 0 ) {
    if ( argc != 5 ) { go_BYE(-1); }
    status = behead(argv[2], argv[3], argv[4]); cBYE(status);
  }
