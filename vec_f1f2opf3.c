...
#pragma omp parallel for
  for ( int tid = 0; tid < nT; tid++ ) { // POTENTIAL CILK LOOP
    ...
    unsigned short     *g1UI2 = NULL,*g2UI2 = NULL,*opUI2 = NULL; 
	...
	if ( status == 0 ) { WHEREAMI; } status = -1; continue;
      ...
      break;
    case I2 : 
      g1UI2 = (unsigned short *)f1_X; g1UI2 += lb;
      g2UI2 = (unsigned short *)f2_X; g2UI2 += lb;
	...
	concat_I2_I2_to_I4(g1UI2, g2UI2, nX, opUI4);
      }
      else { 
	if ( status == 0 ) { WHEREAMI; } status = -1; continue;
      }
