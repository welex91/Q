...
// STOP FUNC DECL
{
  ...
  if ( ( argc == 5 ) && ( strcmp(argv[1], "bootstrap") == 0 ) ) {  
  ...
  status = chk_env_vars(g_docroot); cBYE(status);
  //----------------------------------------------
  if ( ( argc == 4 ) && ( strcmp(argv[1], "init") == 0 ) ) {  
    g_dsk_ddir_id = -1;
    char *dsk_data_dir = argv[2];
    status = init_meta_data(g_docroot, dsk_data_dir);
    cBYE(status);
    goto BYE;
  }
  //------------------------------------------------------
  status = mmap_meta_data(g_docroot, 
			  ...
			  &g_aux_X, &g_aux_nX, &g_dsk_ddir_id);
  cBYE(status);
  // safety check
